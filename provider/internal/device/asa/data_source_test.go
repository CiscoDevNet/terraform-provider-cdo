		...
		Steps: []resource.TestStep{
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.cdo_asa_device.test", "tags.#", strconv.Itoa(len(testAsaDataSource.Tags.Labels))),
					resource.TestCheckResourceAttrWith("data.cdo_asa_device.test", "tags.0", testutil.CheckEqual(testAsaDataSource.Tags.Labels[0])),
					resource.TestCheckResourceAttrWith("data.cdo_asa_device.test", "tags.1", testutil.CheckEqual(testAsaDataSource.Tags.Labels[1])),
					resource.TestCheckResourceAttrWith("data.cdo_asa_device.test", "tags.2", testutil.CheckEqual(testAsaDataSource.Tags.Labels[2])),
				),
			},
		},
	})
